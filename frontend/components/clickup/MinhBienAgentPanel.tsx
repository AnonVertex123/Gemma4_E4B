...
export default function MinhBienAgentPanel({
  ...
  const handleSend = async () => {
    ...
    // V12.1 Fix 2: Minh Biên chat gọi backend qua proxy `/api/diagnose` (không hardcode).
    try {
      const result = await diagnose({ text: t, source: "web" });
      const prefix =
        result.status === "emergency"
          ? "⚠ Cần khám ngay\n\n"
          : result.status === "ask_more"
            ? "Hệ thống cần thêm thông tin để an toàn hơn.\n\n"
            : result.status === "refuse"
              ? "🛑 Không đủ điều kiện an toàn để kết luận.\n\n"
              : "";
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: `a-${Date.now()}`,
          who: "agent",
          text: prefix + (result.message || "Không có thông tin kết quả."),
          ts: new Date().toLocaleTimeString("vi-VN", {
            hour: "2-digit",
            minute: "2-digit",
          }),
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: `a-${Date.now()}`,
          who: "agent",
          text: "Lỗi khi gọi chẩn đoán. Vui lòng thử lại sau.",
          ts: new Date().toLocaleTimeString("vi-VN", {
            hour: "2-digit",
            minute: "2-digit",
          }),
